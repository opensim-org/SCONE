...
namespace scone
{
	...
	public:
		...
		// struct that defines if a controller is active (bitset
		// denotes state(s), leg target should be part of controller)
		...
		struct ConditionalController
		{
			...
			String GetConditionName() const {
				return stringf( "L%dS%s", leg_index, state_mask.to_string().c_str() );
			}
	...
	public:
		...
		const std::vector< LegStateUP >& GetLegStates() const { return m_LegStates; };
