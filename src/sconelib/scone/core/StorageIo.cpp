...
namespace scone
{
	...
	{
		std::ofstream ofs( file.to_string() );
		SCONE_ASSERT_MSG( ofs.good(), "Error opening file " + file.to_string() );
	...
	{
		std::ofstream str( file.to_string() );
		SCONE_ASSERT_MSG( str.good(), "Error opening file " + file.to_string() );
