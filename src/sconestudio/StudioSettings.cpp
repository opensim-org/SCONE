...
namespace scone
{
	...
	xo::settings GetStudioSettings()
	{
		...
		if ( s.empty() )
		{
			s.add( "viewer.background", "Viewer Background", vis::make_from_hsv( 215.0f, 0.05f, 0.666f ) );
			s.add( "viewer.tile1", "Viewer Tile A", vis::make_from_hsv( 215.0f, 0.05f, 0.688f ) );
			s.add( "viewer.tile2", "Viewer Tile B", vis::make_from_hsv( 215.0f, 0.05f, 0.644f ) );
